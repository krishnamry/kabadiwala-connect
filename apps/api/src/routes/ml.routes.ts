...
import { detectScrapWithGemini, STANDARD_RATES } from '../services/geminiDetector';
...
router.post('/classify', upload.single('image'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      const geminiResult = await detectScrapWithGemini(
        req.file.buffer,
        req.file.mimetype,
    ...
    const rateInfo = STANDARD_RATES[detectedCategory] || { rate: 18.0, advice: 'Segregate cleanly for maximum scrap value.' };
