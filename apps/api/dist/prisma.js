...
class SQLitePrismaClient {
    ...
    recyclerProfile = {
        ...
        findMany: async (params) => {
            ...
            return rows.map((r) => this.enrichRecycler(r, params?.include));
        },
        create: async (params) => {
            const id = params.data.id || crypto_1.default.randomUUID();
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, params.data.userId, params.data.facilityName, params.data.cpcbRegNumber, params.data.statePcb || 'Delhi Pollution Control Committee (DPCC)', params.data.latitude ?? 28.5355, params.data.longitude ?? 77.2690, params.data.address || 'Plot 42, Okhla Phase-II, New Delhi', materialsAccepted, offeredRates, params.data.dailyCapacityKg ?? 5000, params.data.pickupAvailable === false ? 0 : 1, params.data.verified === false ? 0 : 1, params.data.rating ?? 4.8, params.data.totalReviewsCount ?? 0);
    ...
    eWasteLot = {
        findMany: async (params) => {
            ...
            query += ' ORDER BY createdAt DESC';
            const rows = this.db.prepare(query).all(...args);
            return rows.map((r) => this.enrichLot(r));
        },
        findUnique: async (params) => {
            let row;
            if (params.where.id) {
        ...
        create: async (params) => {
            ...
            const traceabilityHash = params.data.traceabilityHash || `0x${crypto_1.default.createHash('sha256').update(id + createdAt).digest('hex').slice(0, 16)}`;
            this.db.prepare(`
      ...
      `).run(id, lotCode, params.data.collectorId, params.data.collectorName || 'Collector', params.data.category, params.data.approxWeightKg || 10, params.data.totalItems || 1, params.data.estimatedValue || 0, params.data.askingPrice || params.data.estimatedValue || 0, params.data.minBidAmount || 0, params.data.recyclerOfferedRate || 0, params.data.status || 'AVAILABLE', params.data.highestBid || null, params.data.winningBidId || null, params.data.imageUrl || null, params.data.gpsLat || 28.5685, params.data.gpsLng || 77.2412, params.data.locationAddress || 'Mayapuri Scrap Yard', params.data.locationZone || 'West Delhi', auctionDurationMins, auctionExpiresAt, 0, params.data.recyclerId || null, params.data.recyclerName || null, qrCode, traceabilityHash, params.data.saleTokenNumber || null, null, 0, null, params.data.isCustomLot ? 1 : 0, createdAt, null);
            // Create items if provided
            if (Array.isArray(params.data.items)) {
                for (const it of params.data.items) {
                    const itemId = it.id || crypto_1.default.randomUUID();
                    this.db.prepare(`
    ...
    lotBid = {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, params.data.lotId, params.data.recyclerId, params.data.recyclerName, params.data.bidAmount, params.data.bidPerKg, params.data.status || 'PENDING', params.data.notes || null, createdAt);
    ...
    saleToken = {
        ...
        create: async (params) => {
            ...
            const tokenNumber = params.data.tokenNumber || `KBD-SL-${datePart}-DL01-${crypto_1.default.randomBytes(3).toString('hex').toUpperCase()}`;
            const sha256Signature = params.data.sha256Signature || crypto_1.default.createHash('sha256').update(tokenNumber + params.data.lotId + params.data.netWeightKg).digest('hex');
            this.db.prepare(`
      ...
      `).run(id, tokenNumber, params.data.lotId, params.data.collectorId, params.data.collectorName, params.data.collectorPhone, params.data.collectorAadhaarRef || 'XXXX-XXXX-8921', params.data.recyclerId, params.data.recyclerName, params.data.cpcbRegNumber, params.data.category, params.data.cpcbCategoryCode || 'ITEW2', params.data.grossWeightKg, params.data.tareWeightKg, params.data.netWeightKg, params.data.ratePerKg, params.data.totalAmount, params.data.paymentMode || 'CASH_ON_SPOT', params.data.weighbridgeGpsLat || 28.5355, params.data.weighbridgeGpsLng || 77.2690, params.data.operatorId || 'OP-OKHLA-981', sha256Signature, params.data.eprCredits || params.data.netWeightKg, createdAt);
    ...
    review = {
      ...
      `).run(id, params.data.reviewerId, params.data.targetUserId, params.data.saleTokenId || null, status, params.data.revealedAt || null, params.data.ratingOverall ?? params.data.rating ?? 5, params.data.ratingScaleAcc || null, params.data.ratingPayoutSpd || null, params.data.ratingPurity || null, params.data.reviewText || params.data.comment || null, 1, createdAt);
    ...
    recalculateBayesianScore(userId) {
    ...
    `).all(userId);
        ...
        const R = reviews.reduce((acc, cur) => acc + cur.ratingOverall, 0) / v;
        const m = 5;
        const C = 4.2;
    ...
    chatMessage = {
      ...
      `).all(params.where.contextType, params.where.contextId);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, params.data.senderId, params.data.receiverId || params.data.recipientId || null, params.data.contextType, params.data.contextId, params.data.text || null, params.data.audioUrl || null, params.data.imageUrl || null, 0, createdAt);
            return { id, ...params.data, isRead: false, createdAt };
        },
        markRead: async (contextType, contextId, receiverId) => {
            this.db.prepare(`
