...
class SQLitePrismaClient {
    ...
    saleToken = {
        ...
        create: async (params) => {
            ...
            const sha256Signature = params.data.sha256Signature || crypto_1.default.createHash('sha256').update(tokenNumber + (params.data.lotId || '') + netWeight).digest('hex');
            this.db.prepare(`
      ...
      `).run(id, tokenNumber, params.data.lotId ?? null, params.data.collectorId ?? null, params.data.collectorName ?? 'Suresh Kumar', params.data.collectorPhone ?? '+91 9876543210', params.data.collectorAadhaarRef ?? 'XXXX-XXXX-8921', params.data.recyclerId ?? null, recyclerName, params.data.cpcbRegNumber ?? 'CPCB-REG-2024-DEL-0091', params.data.category ?? 'E-Waste Scrap', params.data.cpcbCategoryCode ?? 'ITEW2', grossWeight, tareWeight, netWeight, rate, total, params.data.paymentMode ?? 'CASH_ON_SPOT', params.data.weighbridgeGpsLat ?? 28.5355, params.data.weighbridgeGpsLng ?? 77.2690, params.data.operatorId ?? 'OP-OKHLA-981', sha256Signature, params.data.eprCredits ?? Number((netWeight * 1.5).toFixed(1)), createdAt);
